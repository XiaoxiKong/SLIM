...
from .noises import Rotation, CropandResize, GaussianBlur, GaussianNoise, ColorJitter, Sharpness, random_int, flip_horizontal
import kornia as K
        ...
        for layer in noise_layers:
            if type(layer) is str:
                if layer == 'Identity':
                    ...
                    self.noise_layers.append(CropandResize((125, 468), (125, 468)))
                elif layer == 'GaussianBlur':
                    self.noise_layers.append(GaussianBlur(10.0))
                elif layer == 'GaussianNoise':
                    self.noise_layers.append(GaussianNoise(0.5))
                elif layer == 'ColorJitter':
                    self.noise_layers.append(ColorJitter())
                elif layer == 'flip_horizontal':
                    self.noise_layers.append(flip_horizontal())
                elif layer == 'DIFF':
                    self.noise_layers.append(DIFF())
                else:
