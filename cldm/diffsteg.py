...
class SecretDecoder(nn.Module):
    def __init__(self, arch='CNN', act='ReLU', norm='none', resolution=256, in_channels=4, secret_len=48):
        super().__init__()
        ...
        if arch=='CNN':
            self.decoder = nn.Sequential(
                ...
                activation(act),
                nn.Conv2d(128, 128, (3, 3), 2, 1),  # 8   nn.Conv2d(128, 128, (3, 3), 2, 1)
                activation(act),
            )
            self.dense = nn.Sequential(
                nn.Linear(512, 512),#inchannel:resolution * resolution * 128 // 32 // 32
                activation(act),
    ...
    def forward(self, image):
        ...
        if self.arch == 'CNN':
            x = x.view(-1, x.shape[1]*x.shape[2]*x.shape[3])# self.resolution * self.resolution * 128 // 32 // 32
            x = self.dense(x)
        return x
