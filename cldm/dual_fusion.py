...
class CrossAttentionInteraction(nn.Module):
    def __init__(self, in_channels=320, n_heads=8, d_head=64,
                 dropout=0.):
        super().__init__()
    ...
    def forward(self, adapter_feature, unet_feature):
        ...
        unet_feature = self.crossAtt_1(fg, unet_fg, mask=None, is_foreground='unet')

        interact_feature = unet_feature.permute(0, 2, 1)

        b, c, h = interact_feature.shape

        interact_feature = interact_feature.reshape(b, c, original_h, original_h)

        new_adapter_feature = self.conv1(interact_feature)
...
class ConvInteraction(nn.Module):
    def __init__(self, in_channels=320, n_heads=8, d_head=64,
                 dropout=0.):
        super().__init__()
