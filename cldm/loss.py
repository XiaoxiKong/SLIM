...
class ImageSecretLoss(nn.Module):
    def __init__(self, recon_type='rgb', recon_weight=1., perceptual_weight=1.0, secret_weight=10., kl_weight=0.000001, logvar_init=0.0, ramp=100000, max_image_weight_ratio=2.) -> None:
        #recon_weight=1., perceptual_weight=1.0, secret_weight=10., kl_weight=0.000001, logvar_init=0.0, ramp=100000, max_image_weight_ratio=2
        super().__init__()
    ...
    def compute_recon_loss(self, inputs, reconstructions):
        if self.recon_type == 'rgb':
            ...
            rec_loss = torch.mm(yuv_loss, self.yuv_scales.to("cuda")).squeeze(1)
        else:
    ...
    def forward(self, inputs, reconstructions, posteriors, secret_gt, secret_pred, global_step):
        ...
        if self.perceptual_weight > 0:
            p_loss = self.perceptual_loss(inputs.contiguous().to("cuda"), reconstructions.contiguous().to("cuda")).mean(dim=[1,2,3])
        ...
        #image_weight = 1 + min(self.max_image_weight,max(0., self.max_image_weight * ( 5e5 - global_step) / self.ramp))
        ...
        rec_mse = torch.pow((inputs - reconstructions).mean(dim=[1, 2, 3]), 2)
        psnr1 = 20 * torch.log10(1 / torch.sqrt(rec_mse))

        loss_dict['bit_acc'] = bit_acc
        loss_dict['psnr'] = psnr1.mean()
