...
class RounderRingMask:
    def __init__(self, size = 64, r_out = RADIUS, x_offset = ANCHOR_X_OFFSET, y_offset = ANCHOR_Y_OFFSET, mode = 'full'):
        ...
        ring_vector = torch.tensor([(200 - i*4) * (-1)**i for i in range(num_rings)])#这种模式常用于建模具有交替特性的环形结构或振荡系统。
        zero_bg_freq[center_x, center_y:center_y+num_rings] = ring_vector#这种操作通常是构建更复杂几何图案（如同心圆环）的第一步。
        ...
        for angle in range(1, 360):
            zero_bg_freq_rot = transforms.functional.rotate(zero_bg_freq, angle=angle)#使用torchvision的rotate函数对图像进行旋转
        ...
        for x in range(size):
            for y in range(size):
                values, count = np.unique(res[:, x, y],  return_counts=True)#使用NumPy的 unique 函数分析3D数组中特定位置的数据分布。
                if len(count) > 2:
...
def SDEditIMG(image, version=1):
    ...
    if version == 1:
        ...
        if pipe2 is None:
            pipe2 = StableDiffusionImg2ImgPipeline.from_pretrained('stabilityai/stable-diffusion-2-1-base', safety_checker=None).to('cuda')
        cur_pipe = pipe2
...
def image_distortion(img1, img2, seed, 
    ...
    if SDEdit is not None:
        if img1 is not None:
            if isinstance(img1, Image.Image):
                ...
                img1 = transform(img1)
                # 添加 batch 维度
                needs_squeeze1 = img1.dim() == 3
                if needs_squeeze1:
                    img1 = img1.unsqueeze(0)
            img1 = SDEditIMG(img1,1)
            if needs_squeeze1:
                img1 = img1.squeeze(0)
        ...
        if isinstance(img2, Image.Image):
            ...
            img2 = transform(img2)
            needs_squeeze2 = img2.dim() == 3
            if needs_squeeze2:
                img2 = img2.unsqueeze(0)
        img2 = SDEditIMG(img2,1)
        if needs_squeeze2:
            img2 = img2.squeeze(0)
    ...
    if crop_scale is not None and crop_ratio is not None:
        if img1 is not None:
            ...
            if isinstance(img1, Image.Image):
                ...
                if needs_squeeze1:
                    img1 = img1.unsqueeze(0)
            img1 = T.RandomCrop(size=(460,460))(img1)
            if needs_squeeze1:
                img1 = img1.squeeze(0)
            img1 = transforms.ToPILImage()(img1)
        ...
        if isinstance(img2, Image.Image):
            ...
            if needs_squeeze2:
                img2 = img2.unsqueeze(0)
        img2 = T.RandomCrop(size=(460, 460))(img2)
        if needs_squeeze2:
            img2 = img2.squeeze(0)
        img2 = transforms.ToPILImage()(img2)
    if gaussian_blur_r is not None:
        ...
        if img1 is not None:
            ...
            img1 = K.augmentation.RandomGaussianBlur((3, 3), (4.0, 4.0), p=1.)(img1)
            if needs_squeeze1:
                img1 = img1.squeeze(0)
            img1 = transforms.ToPILImage()(img1)
        ...
        if isinstance(img2, Image.Image):
            ...
            if needs_squeeze2:
                img2 = img2.unsqueeze(0)
        img2 = K.augmentation.RandomGaussianBlur((3, 3), (4.0, 4.0), p=1.)(img2)
        if needs_squeeze2:
            img2 = img2.squeeze(0)
        img2 = transforms.ToPILImage()(img2)
    ...
    if brightness_factor is not None:
        ...
        img2 = transforms.ColorJitter(brightness=brightness_factor)(img2)"""#original
        if img1 is not None:
            if isinstance(img1, Image.Image):
                # PIL Image -> Tensor
                img1 = transform(img1)
                ...
                if needs_squeeze1:
                    img1 = img1.unsqueeze(0)
            img1 = K.augmentation.ColorJiggle(
                        brightness=(0.9,1.1),
                        contrast=(0.9,1.1),
                        saturation=(0.9,1.1),
                        hue=(-0.1,0.1),
        ...
        if isinstance(img2, Image.Image):
            ...
            if needs_squeeze2:
                img2 = img2.unsqueeze(0)
        img2 = K.augmentation.ColorJiggle(
                        brightness=(0.9,1.1),
                        contrast=(0.9,1.1),
                        saturation=(0.9,1.1),
                        hue=(-0.1,0.1),
...
def get_dataset(dataset_name_or_path):
    if os.path.exists(dataset_name_or_path) and dataset_name_or_path.endswith('.json'):
        with open(dataset_name_or_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
            # Handle COCO-like structure
            ...
            # Handle list of strings
                ...
                dataset = data
                prompt_key = None
    elif 'laion' in dataset_name_or_path:
        dataset = load_dataset(dataset_name_or_path)['train']
        prompt_key = 'TEXT'
    elif 'coco' in dataset_name_or_path:
        # Fallback for coco if not a direct file path, but maybe specific logic was intended
        # Assuming the previous hardcoded path was specific to the user's environment
        # We will try to load it if it exists, otherwise error or empty
        if os.path.exists('fid_outputs/coco/meta_data.json'):
            with open('fid_outputs/coco/meta_data.json') as f:
                dataset = json.load(f)
                dataset = dataset['annotations']
                prompt_key = 'caption'
        else:
             print("Warning: COCO metadata not found at default location.")
             dataset = []
             prompt_key = 'caption'
    else:
        try:
            ...
            prompt_key = 'Prompt'
             ...
             print(f"Warning: Could not load dataset {dataset_name_or_path} from HuggingFace.")
             dataset = []
             prompt_key = None
