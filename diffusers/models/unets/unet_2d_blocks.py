...
class CrossAttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.FloatTensor] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        down_intrablock_additional_residuals: Optional[torch.FloatTensor] = None,
    ) -> torch.FloatTensor:
        if cross_attention_kwargs is not None:
            if cross_attention_kwargs.get("scale", None) is not None:
                logger.warning("Passing `scale` to `cross_attention_kwargs` is depcrecated. `scale` will be ignored.")
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if is_freeu_enabled:
                if down_intrablock_additional_residuals is not None:
                    b22 = down_intrablock_additional_residuals.repeat(1, hidden_states.shape[1], 1, 1)
                    # s11 = down_intrablock_additional_residuals.repeat(1, res_hidden_states.shape[1], 1, 1)
                else:
                    b22 = 1
                    # s11 = 1
            ...
            if (iv == 2) and (down_intrablock_additional_residuals is not None):
                hidden_states = hidden_states * (1+down_intrablock_additional_residuals)
            hidden_states = torch.cat([hidden_states, res_hidden_states], dim=1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for resnet in self.resnets:
            ...
            if is_freeu_enabled:
                if down_intrablock_additional_residuals is not None:
                    # s22 = down_intrablock_additional_residuals.repeat(1,res_hidden_states.shape[1],1,1)
                    b11 = down_intrablock_additional_residuals.repeat(1, hidden_states.shape[1], 1, 1)
                else:
                    # s22 = 1
                    b11 = 1
                hidden_states, res_hidden_states = apply_freeu(
