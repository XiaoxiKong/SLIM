...
def main(args):
    ...
    for i in tqdm(range(start_idx, num_samples)):
        ...
        orig_image_w = outputs_w.images[0]  # Tensor [C, H, W] in range [0, 1]

        # Save image as PNG
        pil_img = to_pil(torch.clamp(orig_image_w, 0.0, 1.0))
        save_path = os.path.join(w_dir, f'sample_{i}.png')
        pil_img.save(save_path)

        # Evaluate accuracy from saved PNG (after compression)
        ...
        decoded_png = Decoder(img_read_tensor.unsqueeze(0))
        decoded_png_bits = torch.argmax(decoded_png, dim=2)
        acc_png = 1 - torch.abs(decoded_png_bits - secret_data).sum().float() / (args.bit_num * 1)
        acc_all_png.append(acc_png.cpu())

        print(f"[sample {i}] acc_png={acc_png.item():.4f}, seed={seed}")

    if acc_all_png:
        print(f"Mean Accuracy(png): {np.mean(acc_all_png)}")
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt_file', default=None,
                        help="Path to prompt file (.json). If not provided, uses built-in demo prompts.")
